  ...
  const chartData = useMemo(() => {
    ...
    data.forEach(row => {
      ...
      if (!weeklyData[weekKey]) {
        weeklyData[weekKey] = { date: weekKey, boi: [], chuva: [], isExtremeRain: false };
      }
      
      if (row.valor_boi_gordo) weeklyData[weekKey].boi.push(row.valor_boi_gordo);
    ...
    return Object.entries(weeklyData)
      .map(([date, values]) => ({
        ...
        boiGordo: values.boi.length > 0 
          ? values.boi.reduce((a, b) => a + b, 0) / values.boi.length 
          : null,
        chuvaLag60d: values.chuva.length > 0 
        ...
        <ComposedChart data={chartData} margin={{ top: 20, right: 60, left: 20, bottom: 20 }}>
          ...
          <Tooltip 
            ...
            formatter={(value: number, name: string) => {
              if (name === 'boiGordo') return [`R$ ${value.toFixed(2)}/@`, 'Boi Gordo'];
              if (name === 'chuvaLag60d') return [`${value.toFixed(1)} mm`, 'Chuva (60d atrás)'];
              return [value, name];
            }}
