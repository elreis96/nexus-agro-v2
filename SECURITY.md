...
- [x] HTTPS obrigatório (HTTPSRedirectMiddleware em produção)
- [x] Trusted Host Middleware (hosts configuráveis via TRUSTED_HOSTS)
- [x] Security Headers (CSP, X-Content-Type-Options, X-Frame-Options, etc.)
- [ ] Monitoramento de logs (Sentry, LogDNA)
- [ ] Backup automático do banco de dados
- [ ] Testes de penetração

### Backup e Monitoramento
- **Backups**: habilitar backups automáticos no Supabase ou rotina externa diária; validar restauração mensalmente.
- **Monitoramento**: configurar Sentry/LogDNA/Datadog com `SENTRY_DSN` e `ENVIRONMENT` para rastrear erros em produção.
- **Alertas**: criar alertas de falha de deploy e erros 5xx (ex.: no provedor de hosting ou via uptime monitor).
