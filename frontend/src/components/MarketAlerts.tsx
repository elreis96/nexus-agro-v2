        ...
        <ScrollArea className="h-[400px] pr-4">
          <div className="space-y-4">
            {alerts.map((alert) => (
              <div
                key={alert.id}
                className="flex items-start gap-4 p-4 rounded-lg bg-muted/30 border border-border/50 hover:border-border/80 transition-colors"
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    <span className="font-semibold text-sm leading-tight">{alert.title}</span>
                    <Badge variant={getAlertBadgeVariant(alert.level)} className="text-xs">
                      {alert.level === 'critical' ? '⚠️ Crítico' :
                       alert.level === 'warning' ? '⚡ Atenção' : 'ℹ️ Info'}
                    </Badge>
                    <span className="text-xs text-muted-foreground/60 ml-auto">
                      {format(new Date(alert.date), "dd MMM yyyy", { locale: ptBR })}
                    </span>
                  </div>
                  ...
                  {alert.level === 'critical' && (
                    <div className="mt-2 text-xs text-destructive font-medium">
                      → Recomendação: Revisar posições e estratégias de proteção
                    </div>
                  )}
                </div>
              </div>
