  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    try {
      if (USE_FASTAPI) {
        ...
        setVolatilidade(volData as ViewVolatilidadeMensal[]);

        type CorrResponse = ViewCorrelacaoDolarJbs[] | { data?: ViewCorrelacaoDolarJbs[] };
        const corrResponse = corrData as CorrResponse;
        const corrList: ViewCorrelacaoDolarJbs[] = Array.isArray(corrResponse)
          ? corrResponse
          : Array.isArray(corrResponse.data)
            ? corrResponse.data
            : [];
        setCorrelacao(corrList);
        
        type LagRow = { data_preco: string; valor_boi_gordo: number; chuva_mm: number };
        const lagRows: LagRow[] = Array.isArray(lagData) ? (lagData as LagRow[]) : [];
        // Map to the full ViewLagChuva60dBoi interface with both original and alias fields
        const mappedLag: ViewLagChuva60dBoi[] = lagRows.map((row) => ({
          data_preco: row.data_preco,
          ano_preco: parseInt(row.data_preco.substring(0, 4)),
          mes_preco: parseInt(row.data_preco.substring(5, 7)),
          valor_boi_gordo: row.valor_boi_gordo,
          chuva_mm_lag_60d: row.chuva_mm,
          data_chuva_original: null,
          // Aliases para componentes legacy
          data_lag: row.data_preco,
          preco_boi: row.valor_boi_gordo,
          chuva_lag: row.chuva_mm,
        }));
        ...
        logger.performance('Analytics fetch completed', startTime, {
          component: 'useAnalytics',
          vol: volData.length, 
          corr: Array.isArray(corrList) ? corrList.length : 0, 
