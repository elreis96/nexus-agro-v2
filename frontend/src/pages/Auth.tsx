...
export default function Auth() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    // Backend validation (security) - validate password before sending to Supabase
    try {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    // Backend validation (security) - validate all signup data before sending to Supabase
    try {
      const API_BASE_URL = import.meta.env.VITE_API_URL || '';
      if (API_BASE_URL) {
        const validateResponse = await fetch(`${API_BASE_URL}/api/auth/validate-signup`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      console.warn('Signup validation endpoint not available, using Supabase validation only:', err);
    }
